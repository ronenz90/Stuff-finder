...
def _smtp_config():
    ...
    if host:
        return {
            ...
            "from_addr": _clean(os.environ.get("SMTP_FROM", "").strip()
...
def _clean(s: str) -> str:
    """
    מנקה פרטי SMTP מתווים נסתרים שנדבקים בהעתקה מאפליקציות:
    רווחים מובילים/עוקבים, תווי כיווניות (RTL/LTR), רווח דק, BOM.
    בלעדי זה, תו נסתר במפתח מפיל את server.login על UnicodeEncodeError.
    """
    if not s:
        return ""
    for ch in ("\u200b", "\u200e", "\u200f", "\u202a", "\u202b", "\u202c",
...
def _smtp_connect(c):
    ...
    user, password = _clean(c["user"]), _clean(c["password"])
    # אזהרה אם נותרו תווים לא-ASCII (סימן להעתקה שגויה / תו עברי בשדה)
    for label, val in (("SMTP_USER", user), ("SMTP_PASSWORD", password)):
        try:
            val.encode("ascii")
        except UnicodeEncodeError:
            print(f"[✗] שים לב: ב-{label} יש תו לא-אנגלי — כנראה נדבק בהעתקה או הוקלד בטעות. תקן את ה-Secret.")
    if c["port"] == 465:
...
def send_email(new_matches: list, pending: list):
    ...
    try:
        for to_addr, matches in by_recipient.items():
            ...
            # קריטי: כותרת בעברית/אימוג'י חייבת קידוד RFC2047 — בלעדיו sendmail
            # נופל על UnicodeEncodeError (ascii) והמייל לא נשלח כלל.
            msg["Subject"] = Header(subject, "utf-8")
            msg["From"] = c["from_addr"]
            msg["To"] = to_addr
            msg.attach(MIMEText(_build_html(matches, pend_n), "html", "utf-8"))
            server.sendmail(c["from_addr"], [to_addr], msg.as_bytes())
            for m in matches:
...
def main():
    ...
    # מצב "בדיקת מייל" — מהריצה הידנית: שולח מייל בדיקה ומדפיס כל שגיאה במלואה
    if os.environ.get("TEST_EMAIL", "").lower() == "true":
        print("🧪 מצב בדיקת מייל...")
        c = _smtp_config()
        if not c:
            print("[✗] לא הוגדרו Secrets של SMTP כלל (SMTP_HOST/USER/PASSWORD).")
            return
        print(f"    שרת: {c['host']}:{c['port']} · משתמש: {c['user']} · שולח: {c['from_addr']}")
        try:
            server = _smtp_connect(c)
            to = os.environ.get("ALERT_EMAIL", "").strip() or c["from_addr"]
            msg = MIMEMultipart("alternative")
            msg["Subject"] = Header("🧪 בדיקת מייל — מערכת מעקב המוצרים", "utf-8")
            msg["From"] = c["from_addr"]
            msg["To"] = to
            msg.attach(MIMEText("<html dir='rtl'><body>אם קיבלת את זה — שליחת המיילים עובדת ✔</body></html>", "html", "utf-8"))
            server.sendmail(c["from_addr"], [to], msg.as_bytes())
            server.quit()
            print(f"[✓] מייל בדיקה נשלח אל {to} — בדוק גם בספאם.")
        except Exception:
            import traceback
            print("[✗] הבדיקה נכשלה. השגיאה המלאה:")
            traceback.print_exc()
        return
    ...
    for product in products:
        ...
        for norm, r in candidates.items():
            ...
            # התראה אוטומטית: כל מילות השם קיימות + דף מכירה + לא כתבה/מוזיאון.
            # כל השאר (50-99%, או חדשותי) — לסינון AI בלוח הבקרה.
...
def notify_unsent(found: dict, pending: list):
    """
    שולח מייל על כל מציאה עם notified=False ומסמן אותה אחרי שליחה מוצלחת.
    ...
    if not unsent and not pending:
        print("אין מציאות חדשות לדווח.")
        return
    try:

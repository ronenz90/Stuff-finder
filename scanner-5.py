...
def _send_via_formsubmit(to_addr: str, subject: str, html: str) -> bool:
    """
    שולח מייל דרך FormSubmit (https://formsubmit.co) — חינמי, בלי סיסמאות/SMTP.
    FormSubmit שולח מהשרת המאומת שלו, כך שאין בעיית התחזות מול Gmail.
    בפעם הראשונה לכל נמען הוא שולח מייל אימות חד-פעמי שצריך ללחוץ בו.
    מחזיר True אם הבקשה התקבלה.
    """
    ...
    try:
        resp = requests.post(
            ...
            json={
                "_subject": subject,
                "_template": "box",
                "message": html,  # FormSubmit שולח את הטקסט; ה-HTML יופיע כתוכן
            },
            timeout=30,
        )
        if resp.status_code == 200 and resp.json().get("success") in (True, "true"):
            return True
        print(f"[✗] FormSubmit החזיר: {resp.status_code} · {resp.text[:200]}")
        return False
    except Exception as e:
        print(f"[✗] שגיאת FormSubmit: {e}")
        return False


def send_email(new_matches: list, pending: list):
    """
    שולח התראות לפי הנמען של כל מוצר דרך FormSubmit:
    ...
    for to_addr, matches in by_recipient.items():
        ...
        if _send_via_formsubmit(to_addr, subject, _build_html(matches, pend_n)):
            for m in matches:
...
def main():
    ...
    if os.environ.get("TEST_EMAIL", "").lower() == "true":
        ...
        if ok:
            print(f"[✓] הבקשה נשלחה ל-FormSubmit עבור {to}.")
            print("    ⚠️ בפעם הראשונה: FormSubmit שולח מייל אימות חד-פעמי —")
            print("    חפש מייל מ-FormSubmit (גם בספאם) ולחץ על הקישור לאישור.")
            print("    אחרי האישור, כל המיילים הבאים יגיעו ישירות.")
        else:

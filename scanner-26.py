...
def build_queries(product: dict) -> list:
    """בונה שאילתות לכל שפה + מילות מפתח + חיפוש ממוקד ב-AliExpress."""
    queries = []
    ...
    for kw in product.get("keywords", []):
        queries.append(kw)
    # חיפוש ממוקד ב-AliExpress (פריטים בודדים; עמודי הסיטונאות ייחסמו בהמשך)
    if en_name:
        queries.append(f"{en_name} site:aliexpress.com")
    return queries

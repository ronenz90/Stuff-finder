...
# אתרים לחיפוש ממוקד, לפי קטגוריה. ניתן לערוך את הרשימות חופשית.
# לכל אתר תתווסף שאילתה "<שם המוצר> site:<דומיין>" (דרך DuckDuckGo בלבד).
SITE_GROUPS = {
    # עתיקות, יודאיקה, אספנות ומכירות פומביות
    "collectibles": [
        "bidspirit.com",         # בית מכירות פומביות ישראלי — יודאיקה ועתיקות
        "kedem-auctions.com",    # קדם — מכירות פומביות ישראליות
        "catawiki.com",          # מכירות אספנות אירופאיות
        "liveauctioneers.com",   # אגרגטור מכירות פומביות עולמי
        "invaluable.com",        # מכירות פומביות ואמנות
        "worthpoint.com",        # מאגר עתיקות ומחירים
        "rubylane.com",          # שוק עתיקות
        "1stdibs.com",           # עיצוב ועתיקות יוקרה
        "etsy.com",              # וינטג' ועבודות יד
    ],
    # מוצרי צריכה, אלקטרוניקה וכלי בית
    "consumer": [
        "zap.co.il",             # השוואת מחירים ישראלית
        "ksp.co.il",             # אלקטרוניקה ישראל
        "amazon.com",
        "walmart.com",
        "aliexpress.com",
        "banggood.com",
...
def sites_for(product: dict) -> list:
    """
    מחזיר את רשימת האתרים לחיפוש לפי הקטגוריה שנבחרה למוצר.
    site_group: 'collectibles' | 'consumer' | 'all' | 'none' (ברירת מחדל: all)
    """
    group = (product.get("site_group") or "all").lower()
    if group == "none":
        return []
    if group in SITE_GROUPS:
        return SITE_GROUPS[group]
    # 'all' או ערך לא מוכר — הכל
    return SITE_GROUPS["collectibles"] + SITE_GROUPS["consumer"]


def build_queries(product: dict) -> list:
    """
    בונה שאילתות. מחזיר רשימת (שאילתה, is_primary).
    is_primary=True → יורץ בכל הספקים כולל APIs מוגבלים (Google 100/יום).
    is_primary=False → יורץ ב-DuckDuckGo בלבד (ללא הגבלה), כדי לא לשרוף מכסות.
    """
    queries = []
    ...
    for lang, name in names.items():
        if not name:
            continue
        if lang == "he":
            queries.append((f"{name} למכירה", True))
            queries.append((f"{name} מכירה פומבית", False))
        else:
            queries.append((f"{name} for sale", True))
            queries.append((f"{name} auction", False))
    for kw in product.get("keywords", []):
        queries.append((kw, False))
    # חיפושים ממוקדים באתרים לפי הקטגוריה — DuckDuckGo בלבד
    # (אין טעם לשלוח "site:zap.co.il" ל-API של eBay/Etsy)
    if en_name:
        for site in sites_for(product):
            queries.append((f"{en_name} site:{site}", False))
    # לשם עברי — חיפוש ממוקד באתרים הישראליים בלבד
    he_name = names.get("he", "")
    if he_name:
        for site in sites_for(product):
            if site.endswith(".co.il"):
                queries.append((f"{he_name} site:{site}", False))
    return queries
...
# --- Etsy API (חינמי: דורש מפתח אפליקציה, ~10,000 קריאות/יום) ------------


def search_etsy(query: str) -> list:
    """
    חיפוש ב-Etsy — שוק ענק לוינטג', עתיקות ופריטי אספנות.
    הנקודה החזקה: מחזיר רק מודעות פעילות עם מחיר (pre_verified),
    בדיוק כמו eBay. דורש Secret אחד: ETSY_API_KEY.
    """
    ...
    try:
        resp = requests.get(
            ...
            params={"keywords": query, "limit": RESULTS_PER_QUERY, "sort_on": "created"},
            timeout=REQUEST_TIMEOUT,
        )
        ...
        for it in resp.json().get("results", []):
            ...
            price_txt = f"{amount / divisor:.2f} {cur}" if amount else ""
            out.append({
                "title": it.get("title", ""),
                "url": it.get("url") or f"https://www.etsy.com/listing/{it.get('listing_id','')}",
                "snippet": f"מחיר: {price_txt} · Etsy · {(it.get('description') or '')[:120]}",
                "source": "etsy",
                "pre_verified": True,   # ה-API מחזיר רק מודעות פעילות
            })
        return out
    except Exception as e:
        print(f"  [!] Etsy נכשל עבור '{query}': {e}")
        return []


def search_web(query: str, is_primary: bool = True) -> list:
    """
    מריץ את השאילתה בספקים המתאימים.
    שאילתה עיקרית → כל הספקים (כולל APIs עם מכסה מוגבלת).
    שאילתה משנית → DuckDuckGo בלבד (ללא הגבלה), כדי לחסוך במכסות.
    """
    results = []
    if is_primary:
        results += search_ebay(query)
        results += search_etsy(query)
...
def verify_listing(url: str) -> dict:
    ...
    מחזיר גם את טקסט הדף (page_text) לבדיקות נוספות כמו יעד משלוח.
    """
    verdict = {"alive": False, "available": False, "is_sale_page": False,
               "reason": "", "page_text": ""}
    try:
...
# דפי חיפוש/קטגוריה — לא דף מוצר ספציפי (מתחזים: חיים, עם מחירים וכפתורי קנייה)
SEARCH_PAGE_PATTERNS = [
    r"/sch/", r"/sch\?", r"/s\?", r"/search", r"/browse", r"/category", r"/categories",
    r"/c/", r"/shop/", r"/collections?/", r"/tag/", r"/results",
    r"[?&](q|query|search|keyword|kw|searchtext|_nkw)=",
    r"/w/", r"/wholesale",
]
SEARCH_PAGE_RE = re.compile("|".join(SEARCH_PAGE_PATTERNS), re.IGNORECASE)
# כותרות אופייניות לדפי חיפוש
SEARCH_TITLE_RE = re.compile(
    r"(for sale\s*\|\s*ebay|search results|results for|"
    r"\d+\+?\s*(items?|results|products)|shop .* online|"
    r"תוצאות חיפוש|מוצרים בקטגוריה)",
    re.IGNORECASE,
)


def is_search_page(url: str, title: str) -> bool:
    """
    מזהה דפי חיפוש/קטגוריה שאינם מוצר ספציפי.
    למשל 'shabbat for sale | eBay' עם /sch/ ב-URL — דף תוצאות, לא מודעה.
    """
    if SEARCH_PAGE_RE.search(url or ""):
        return True
    if SEARCH_TITLE_RE.search(title or ""):
        return True
    return False


# דפי מוצר אמיתיים — תבניות URL מוכרות (עוקף זיהוי-שווא)
PRODUCT_URL_RE = re.compile(
    r"(/itm/|/item/|/listing/|/product/|/products/|/dp/|/p/\d|/i/\d|/ip/)",
    re.IGNORECASE,
)
# ── סינון יעד משלוח ──────────────────────────────────────────────
# מידע המשלוח נמצא בעומק דף המוצר ובכל אתר בניסוח אחר. לכן ההכרעה
# היא בשלוש דרגות: שולח / לא שולח / לא ידוע (יורד לסינון AI, לא נפסל).

WORLDWIDE_RE = re.compile(
    r"(worldwide shipping|ships? worldwide|world\s?wide delivery|"
    r"international shipping|ships? internationally|global shipping|"
    r"we ship (to )?(all over the world|worldwide|internationally)|"
    r"משלוח לכל העולם|משלוח בינלאומי|שילוח עולמי)",
    re.IGNORECASE,
)
NO_INTL_RE = re.compile(
    r"(no international shipping|does not ship internationally|"
    r"domestic shipping only|ships? only (with)?in |"
    r"we do not ship outside|shipping within \w+ only|"
    r"אין משלוח בינלאומי|משלוח בארץ בלבד)",
    re.IGNORECASE,
)


def shipping_verdict(page_text: str, country_code: str, country_en: str):
    """
    מחזיר True (שולח) / False (לא שולח) / None (לא ידוע).
    שמרני בכוונה: פוסל רק כשיש הדרה מפורשת, כדי לא לפספס מוצרים.
    """
    if not country_en:
        return None
    txt = page_text[:400_000]
    c = re.escape(country_en)

    # 1. הדרה מפורשת של היעד — פסילה ודאית
    excl = re.compile(
        rf"(do(es)?n'?t ship to {c}|do(es)? not ship to {c}|won'?t ship to {c}|"
        rf"cannot ship to {c}|can'?t ship to {c}|no shipping to {c}|"
        rf"not available (for shipping )?(in|to) {c}|"
        rf"excluding[^.]{{0,80}}{c}|excludes[^.]{{0,80}}{c}|"
        rf"{c}[^.]{{0,40}}(not available|excluded|unavailable))",
        re.IGNORECASE,
    )
    if excl.search(txt):
        return False

    # 2. הצהרה שאין משלוח בינלאומי כלל — פסילה
    #    (חייב להיבדק *לפני* הסימנים החיוביים: "No international shipping"
    #     מכיל את הצירוף "international shipping" ואחרת היה מזוהה כחיובי)
    if NO_INTL_RE.search(txt):
        return False

    # 3. אזכור מפורש של משלוח ליעד — אישור
    ships_to = re.compile(rf"(ships? to {c}|delivery to {c}|shipping to {c}|available in {c})",
                          re.IGNORECASE)
    if ships_to.search(txt):
        return True

    # 4. משלוח עולמי/בינלאומי — אישור
    if WORLDWIDE_RE.search(txt):
        return True

    return None  # לא ידוע
# אתרי סיטונאות / מינימום הזמנה — לא רלוונטי לאספן (מוכרים בכמויות, לא פריט בודד)
WHOLESALE_DOMAINS = [
    "alibaba.com", "made-in-china.com", "dhgate.com", "globalsources.com",
    "1688.com", "indiamart.com", "ec21.com", "tradeindia.com", "wholesale",
    "aliexpress.com/wholesale", "thomasnet.com", "go4worldbusiness.com",
    "exporthub.com", "ecplaza.net", "tradekey.com",
]
WHOLESALE_RE = re.compile("|".join(re.escape(d) for d in WHOLESALE_DOMAINS), re.IGNORECASE)
# ביטויים בתוכן שמעידים על מינימום הזמנה
MOQ_RE = re.compile(
    r"(minimum order|min\.?\s*order|moq|min\.?\s*quantity|"
    r"pieces?\s*\(min\.?\s*order\)|/\s*piece|per\s*piece|"
    r"מינימום הזמנה|כמות מינימלית|הזמנה מינימלית)",
    re.IGNORECASE,
)
...
def extract_price_usd(text: str, fx: dict):
    ...
    מלכודת עלי אקספרס: מחיר טווח ("$2-50") או "from $2" הוא הווריאנט הזול
    ביותר ולא המוצר האמיתי — במקרה כזה מחזיר None (מחיר לא ודאי).
    """
    if not fx:
        return None, None
    # זיהוי תמחור טווח/פתיח — לא אמין
    if re.search(r"[$₪€£]\s?\d[\d,.]*\s?[-–—]\s?[$₪€£]?\s?\d", text) or \
       re.search(r"\b(from|starting at|as low as|החל מ|מחיר החל)\b", text, re.IGNORECASE):
        return None, None  # מחיר טווח — מסמן כלא ודאי, ה-AI/הסינון יטפל
...
def main():
    ...
    for product in products:
        ...
        candidates = {}
        for q, is_primary in build_queries(product):
            print(f"  🔍 {q}{'' if is_primary else ' (DDG בלבד)'}")
            for r in search_web(q, is_primary):
                url = r["url"]
                if not url:
                    continue
        ...
        for norm, r in candidates.items():
            ...
            # חסימת אתרי סיטונאות / מינימום הזמנה — לא רלוונטי לאספן, נזרק לגמרי
            if WHOLESALE_RE.search(r["url"]) or WHOLESALE_RE.search(r["title"]):
                print(f"    [סיטונאות] {r['url'][:70]} — נדחה (מינימום הזמנה)")
                seen.add(norm)
                continue

            is_newsy = bool(NEWS_RE.search(f"{r['url']} {r['title']}"))
            # דף חיפוש/קטגוריה? (אלא אם ה-URL הוא תבנית מוצר מוכרת כמו /itm/)
            is_search = is_search_page(r["url"], r["title"]) and not PRODUCT_URL_RE.search(r["url"])
            if is_search:
                print(f"    [דף חיפוש] {r['url'][:70]} — לא דף מוצר ספציפי")
            ...
            # --- בדיקת יעד משלוח ---
            ship_ok = None  # None = לא ידוע, True = שולח, False = לא שולח
            ship_code = (product.get("ship_to") or "").strip()
            ship_en = (product.get("ship_to_en") or "").strip()
            if ship_code and verdict.get("page_text"):
                ship_ok = shipping_verdict(verdict["page_text"], ship_code, ship_en)
                if ship_ok is False:
                    print(f"      🚫 לא שולח ל{product.get('ship_to_he') or ship_en}")
                elif ship_ok is None:
                    print(f"      ❔ יעד משלוח לא ברור — יורד לסינון AI")
            ...
            entry = {
                ...
                "price_raw": price_raw,
                "ships_to_target": ship_ok,   # True/False/None
                "ship_to_label": product.get("ship_to_he") or ship_en,
                "notified": False,  # יסומן True רק אחרי שליחת מייל מוצלחת
                "found_at": datetime.now(timezone.utc).isoformat(),
            }

            # זיהוי מינימום הזמנה בכותרת/תקציר (גם באתרים רגילים) — לא מתאים לאספן
            has_moq = bool(MOQ_RE.search(f"{r['title']} {r['snippet']}"))
            if has_moq:
                print(f"      📦 זוהה מינימום הזמנה — יורד לסינון AI")

            # התראה אוטומטית: כל מילות השם + דף מכירה + לא כתבה + לא מעל התקציב +
            # לא מינימום הזמנה + דף מוצר ספציפי + שולח ליעד (או שאין דרישת יעד).
            ship_blocks = (ship_code and ship_ok is not True)  # רק True מאשר אוטומטית
            if (score >= 100 and verdict["is_sale_page"] and not is_newsy
                    and not over_budget and not has_moq and not is_search
                    and not ship_blocks):
                new_matches.append(entry)
                seen.add(norm)
            elif is_search:
                # דף חיפוש — חסר ערך גם לסינון AI, נזרק לגמרי
                seen.add(norm)
            elif ship_ok is False:
                # הדרה מפורשת של היעד — אין טעם אפילו בסינון AI
                print(f"      ✗ נפסל: לא שולח ליעד")
                seen.add(norm)
            else:
    ...
    if not isinstance(old_pending, list):
        old_pending = []
    # מיזוג עם מניעת כפילויות לפי URL (שלא יופיע אותו מועמד פעמיים בתור)
    merged, seen_p = [], set()
    for item in old_pending + pending:
        u = (item.get("url") or "").rstrip("/").lower()
        if u and u not in seen_p:
            seen_p.add(u)
            merged.append(item)
    save_json(PENDING_FILE, merged[-200:])

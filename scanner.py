...
# אתרי אספנות/עתיקות לחיפוש ממוקד — ניתן לערוך את הרשימה לפי תחומי העניין שלך.
# לכל אתר תתווסף שאילתה "<שם המוצר> site:<דומיין>" בכל סריקה.
COLLECTIBLE_SITES = [
    "bidspirit.com",      # בית מכירות פומביות ישראלי — יודאיקה ועתיקות
    "catawiki.com",       # מכירות אספנות אירופאיות
    "etsy.com",           # וינטג' ועתיקות
...
def build_queries(product: dict) -> list:
    ...
    # חיפושים ממוקדים באתרי מכירות ואספנות — DuckDuckGo בלבד
    # (אין טעם לשלוח "site:bidspirit.com" ל-API של eBay/Etsy)
    if en_name:
        queries.append((f"{en_name} site:aliexpress.com", False))
        for site in COLLECTIBLE_SITES:
